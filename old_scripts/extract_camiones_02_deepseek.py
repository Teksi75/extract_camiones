...
Resultado en Excel con 2 columnas (Receptor/Indicador) y corrección de campos e=dd=dt.
...
def extraer_camiones_por_ot(ot: str, user: str, pwd: str, mostrar_navegador: bool = True, 
    ...
    con las columnas en formato de 2 columnas (Receptor/Indicador).
    """
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for idx, href in enumerate(instrument_links, start=1):
                ...
                # CORRECCIÓN: e = dd=dt
                # Para el receptor, si tenemos dd_dt pero no e, usamos dd_dt para e
                dd_dt_rec = (rec_model.get("dd_dt") or "").strip()
                e_rec = (rec_model.get("e") or "").strip()
                if not e_rec and dd_dt_rec:
                    e_rec = dd_dt_rec
                
                # Para el indicador, si tenemos dd_dt pero no e, usamos dd_dt para e
                dd_dt_ind = (ind_model.get("dd_dt") or "").strip()
                e_ind = (ind_model.get("e") or "").strip()
                if not e_ind and dd_dt_ind:
                    e_ind = dd_dt_ind
                ...
                # Crear fila en formato de 2 columnas (Receptor/Indicador)
                fila = {
                    # Información general
                    ...
                    # COLUMNA RECEPTOR
                    "Receptor - Fabricante": fab_rec,
                    "Receptor - Marca": marca_rec,
                    "Receptor - Modelo": modelo_rec,
                    "Receptor - N° de serie": serie_rec,
                    "Receptor - Cód ap. mod.": codap_rec,
                    "Receptor - Origen": origen_rec,
                    "Receptor - e": e_rec,
                    "Receptor - dd=dt": dd_dt_rec,
                    "Receptor - máx": max_rec,
                    "Receptor - mín": min_rec,
                    "Receptor - clase": clase_rec,
                    "Receptor - N° de Aprobación Modelo": naprob_rec,
                    "Receptor - Fecha de Aprobación Modelo": faprob_rec,

                    # COLUMNA INDICADOR
                    "Indicador - Tipo": "electrónica",
                    "Indicador - Fabricante": fab_ind,
                    "Indicador - Marca": marca_ind,
                    "Indicador - Modelo": modelo_ind,
                    "Indicador - N° de serie": serie_ind,
                    "Indicador - Código Aprobación": codap_ind,
                    "Indicador - Origen": origen_ind,
                    "Indicador - N° de Aprobación Modelo": naprob_ind,
                    "Indicador - Fecha de Aprobación Modelo": faprob_ind,
                }
...
# =========================
# Excel - Formato 2 Columnas (CORREGIDO)
# =========================
...
COLUMNS_ORDER = [
    ...
    "Receptor - mín", 
    "Receptor - clase",
    "Receptor - N° de Aprobación Modelo",
    "Receptor - Fecha de Aprobación Modelo",
    
    # Indicador
    "Indicador - Tipo",
    "Indicador - Fabricante",
    "Indicador - Marca",
    "Indicador - Modelo",
    "Indicador - N° de serie",
    "Indicador - Código Aprobación", 
    "Indicador - Origen",
    "Indicador - N° de Aprobación Modelo",
    "Indicador - Fecha de Aprobación Modelo"
]
...
def exportar_verificacion(df: pd.DataFrame, ruta: Path) -> Path:
    ...
    with pd.ExcelWriter(ruta, engine="xlsxwriter") as writer:
        # Escribir datos normalmente
        df.to_excel(writer, sheet_name="Verificación", index=False, startrow=1)
        wb = writer.book
        ...
        # Formato para encabezados de sección (CORREGIDO - sin insert_row)
        ...
        # Escribir encabezados de sección en la fila 0 (CORREGIDO)
        ws.merge_range(0, 0, 0, 8, "INFORMACIÓN GENERAL", fmt_section)
        ws.merge_range(0, 9, 0, 21, "RECEPTOR", fmt_section)
        ws.merge_range(0, 22, 0, 30, "INDICADOR", fmt_section)

        # Re-escribir los encabezados de columna en la fila 1 con formato
        for i, col in enumerate(COLUMNS_ORDER):
            ws.write(1, i, col, fmt_header)

        # Aplicar formato a los datos (comenzando desde la fila 2)
        for row_num in range(2, len(df) + 2):
            for col_num in range(len(COLUMNS_ORDER)):
                try:
                    value = df.iloc[row_num - 2, col_num]
                    ws.write(row_num, col_num, value, fmt_data)
        ...
        for i, col in enumerate(COLUMNS_ORDER):
            try:
                ...
                max_len = min(max(15, max_len + 2), 50)
        ...
        ws.set_row(0, 25)  # Fila de sección
        ws.set_row(1, 30)  # Fila de encabezados
...
class MetroWebExtractorApp:
    ...
    def run_extraction(self):
        ...
        try:
            ...
            self.update_status("NOTA: El Excel se generará en formato de 2 columnas (Receptor/Indicador)")
            ...
            # Generar Excel
            self.update_status("Generando archivo Excel en formato de 2 columnas...")
            ...
            self.update_status(f"Formato: 2 columnas (Receptor/Indicador)")
            ...
            self.root.after(0, lambda: messagebox.showinfo(
                ...
                f"Formato: 2 columnas (Receptor/Indicador)\n"
            ...
            import traceback
            self.update_status(f"Detalles: {traceback.format_exc()}")
